...
%V3 第三版：将sigma矩阵中lamda更改为:总和为1->均值为1（效果差不多，有时候前者效果好，有时候后者效果好）
...
%V5 第五版：考虑更新缺失值时可能出现多个最大值，这样一行会出现多个1，所以加入了随机选择最大值赋值
%           效果：部分不变，因为不存在重复元素；其他比V4下降，因为抹去了其他最大值的权重，损失了信息

%V6 第六版：将更新sigma(lamda)处计算X1、X2、X3处的错误改正（j->i），部分结果有所提升
%           （对于代理函数法）考虑某些簇不包含元素的情况，额外将其权重调整为包含元素的簇的权重的最小值，再归一化
%           （否则，在更新sigma处，X1X2X3可能为0，导致lamda出现NAN，二次规划出现不是有限的值，报错）

%V7 第七版：感觉sigma(lamda)的优化过程可能存在问题，换用二次规划解决
%           若采用二次规划，则对基聚类器Hoi的性质有所要求，
%           则需要将对缺失值的更新更换为出现多个最大值，随机分配，方便后续优化，
%           防止在更新sigma(lamda)是出现一个基聚类器中某个元素同时出现在两个簇中
%           效果差不多

%在更新sigma处，出现了问题
%1.X1X2X3可能为0，导致lamda出现NAN，二次规划出现不是有限的值，报错
%2.理论上应该在更新公式上乘上原值，之前没加，但是加了之后反而性能下降 还是存在问题

%优势：可以处理簇数不一致情况，在这个样本集中没有体现（PCE也可以处理？）
%无须参数设置
...
for iternum=1:maxiter%迭代次数
    ...
    for i=1:ensemblesize
        ...
        for j = idx_u{i}%选取第i个基聚类器中的缺失行
            ...
            %如果最大值不止一个呢？？ 必须考虑，不然后续优化无法使用基聚类器Hoi的性质
            ...
            [maxValue, maxIndex] = max(Hu{i}(currentrow, :));
            %防止有多个最大值，选择一个
            maxIndices = find(Hu{i}(currentrow, :) == maxValue);
            randomIndex = maxIndices(randi(numel(maxIndices)));
            %将选取位置赋值为1，其他位置赋值为 0
            Hu{i}(currentrow, :) = zeros(1, numel(Hu{i}(currentrow, :)));
            Hu{i}(currentrow, randomIndex) = 1;
        end
    end
    ...
    %     end
    % 
    %     %只需要计算对角线的取值(后续这里可以优化复杂度，Temp是对角矩阵)
    %     %这里修复了初版错误        
    %     X1=Hoi{i}'*A*Hoi{i};
    %     X2=Hoi{i}'*B*Hoi{i};
    %     Temp=Hoi{i}'*Hoi{i};
    %     X3=0.5*alpha(i)*Temp*diag(lamda{i})*Temp;
    % 
    %     for j=1:cnum(i)
    %         %lamda{i}(j)=lamda{i}(j)*X1(j,j)/(X2(j,j)+X3(j,j));
    %         lamda{i}(j)=X1(j,j)/(X2(j,j)+X3(j,j));
    %     end
    % 
    %     %人为规定，没有元素的簇的权重会被认为是非0权重的簇中的最小权重
    %     min_lamda = min(lamda{i});
    %     for j=1:cnum(i)
    %         if isnan(lamda{i}(j))
    %             lamda{i}(j)=min_lamda;
    %         end
    ...
    %     lamda{i} = cnumD(i) * lamda{i} ./ lamda_sum;
    %     %lamda{i} = lamda{i} ./ lamda_sum;
    % end

    %update sigma(lamda) Solution2
    %依次更新各个基聚类器中簇的权重
    for i=1:ensemblesize
        %计算当前B取值
        B=zeros(datasize, datasize);
        ...
        lamda{i}=quadprog(G,f,[],[], ones(1,cnum(i)),cnum(i), zeros(cnum(i),1),ones(cnum(i),1), lamda{i},opt);
        %lamda{i}=quadprog(G,f,[],[], ones(1,cnum(i)),1, zeros(cnum(i),1),ones(cnum(i),1), lamda{i},opt);
    end
...
%已得到预测CA矩阵 A
K=cnum(1);%希望得到的簇数(?) 后续簇数不一样，可以在这里换一个计算簇数的方式
